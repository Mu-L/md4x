...
- **libc "fast paths" must stay target-gated, and only NUL-free-safe libc calls belong in the parser:** upstream md4c reaches for `strcspn()` in `md_analyze_line`'s line-end scan only under `#if defined __linux__`, because it is a win exclusively where libc ships a vectorized implementation. `blocks.zig` spells the real condition — `use_libc_strcspn = builtin.target.isGnuLibC()` — and runs the manual unrolled loop everywhere else. **Do not widen it**: `os.tag == .linux` sweeps in the two `-musl` napi targets, and `abi.isGnu()` is also true for `windows-gnu` (mingw, not glibc). Think twice before calling any `<string.h>` function from the parser at all. Neither wasi-libc nor Zig's bundled musl ships `strcspn.c`/`strspn.c`/`strstr.c`, so Zig's own fallbacks (`lib/c/string.zig`) get linked — and every one of them `std.mem.span()`s its argument first, i.e. runs `strlen()`. The parser's buffers are **not NUL-terminated**, so that reads past the end of the document until it happens to meet a zero byte. Un-gating this one made the parse O(lines × bytes): ~1.8× on `md_parse` for the wasm build and **over 500×** on a 32 KB/800-line document for `linux-*-musl`. The parser's other libc externs (`memcmp`, `memmove`, `qsort`, `bsearch`) all take an explicit length and are fine. See the CHANGELOG entry.
