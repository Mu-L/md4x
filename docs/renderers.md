...
**Entities are resolved, not passed through as source.** `## A &amp; B` is `["h2",{"id":"a--b"},"A & B"]`, and the same holds for a link `href`/`title` and an image `src`/`alt`. It used to hand back the source spelling, so one `parseAST` result answered "what does this heading say" two different ways — the text child said `A &amp; B` while the `id` was slugged from the resolved `A & B` — and every consumer that is not md4x's own HTML renderer had to reimplement entity resolution or ship the bug (undocs rendered each `&copy;`/`&mdash;`/`&nbsp;` as its own spelling until it was worked around downstream). An unrecognized entity stays verbatim and a codepoint that is not a Unicode scalar value folds to U+FFFD, both matching the HTML renderer; content that is literal by definition — code spans, fenced code, math, raw HTML runs — is untouched, since the parser never types it as an entity. The resolver is `slug.resolveEntity`, the same one behind heading text.
...
**Two paragraphs the tree drops.** Markdown wraps loose block content in a paragraph unconditionally, and in two places that paragraph describes the source rather than the document. A paragraph holding only MDC components and whitespace is spliced out, so a component written on its own line lands at block level (matching `markdown-it-mdc`) while a mid-sentence one keeps its paragraph; and a `template` slot body that is exactly one paragraph is unwrapped, unless that paragraph carries trailing `{attrs}` — those live on the paragraph node and `template` is a slot marker the consumer unwraps when mounting, so unwrapping would drop them and put the tree at odds with the HTML renderer. Both are the rule md4x already applies to a tight list item, which renders as `["li", {}, "one"]` — and which keeps its attributes too, as `- one {.x}` renders as `["li", { "class": "x" }, "one"]`. These live in the AST renderer, not the parser: every other renderer emits real markup, where the wrapper is harmless or required.

**Autolinks are marked.** An `a` node the author never spelled as a link carries `"autolink": true` — both the bracketed form (`<https://x>`) and the permissive ones (`www.x`, a bare URL, a bare email), which is exactly the parser's `is_autolink` detail bit. It is one flag for all four spellings: the parser synthesizes the `http://` / `mailto:` prefix and keeps no record of which form produced the href, so a consumer can tell an autolink from `[text](href)` but cannot round-trip the original syntax from the AST alone. Emitted only when true.
