...
::callout
Text nodes in Comark are simple strings, similar to HTML text nodes. To add custom styles, attributes, or metadata to specific parts of text, use the [Span Attributes](/syntax/attributes#span-attributes) syntax: `Hello [world]{data-world="earth" style="color: blue"}`.
...
Bare URLs like `https://example.com` are automatically converted into clickable links. Disable this with the [`linkify`](/reference/parse#options) option (`{ linkify: false }`).
...
The [alerts plugin](/plugins/defaults/alert) is built-in and transforms special blockquotes into styled callout blocks. Place an alert marker on the first line of a blockquote:
...
Comments are represented in the [document model](/getting-started/document-model#comment-nodes) as a tuple with `null` as the tag:
